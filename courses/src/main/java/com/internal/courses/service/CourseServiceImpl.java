...
public class CourseServiceImpl implements  CourseService{
     ...
     */
    @Override
    public List<CourseResponseDTO> getCoursesByTechnologyAndDurationRange(String technology, String durationFrom, String durationTo, String token) {
        int fromTime=Integer.parseInt(durationFrom.substring(0,2));
        System.out.println(fromTime);
        int toTime=Integer.parseInt(durationTo.substring(0,2))+1;
        ...
        if (!tokenResponse.getIsTokenValid() && tokenResponse.getIsExpired()) {
            ...
            return courseRepository.findByTechnologyAndDurationRange(technology, fromTime, toTime).stream()
                    .map(courseMapper::toDto)
                    .collect(Collectors.toList());
        }
    }
